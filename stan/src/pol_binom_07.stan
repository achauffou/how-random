functions{
  ...
  real partial_sum(
    ...
    real sigma_gamma, vector zgamma_pla, vector zgamma_pol, vector lambda, 
    vector SS
  ) {
    real lp = 0.0;
    ...
    for (i in 1:(end - start + 1)) {
      lp += binomial_logit_lpmf(
        y[i] | n[i], alpha + beta[site_id[i]] + sigma_gamma * zgamma_pla[pla_id[i]] * 
        zgamma_pol[pol_id[i]] + lambda[site_id[i]] * SS[start + i - 1]
      );
    }
    return lp;
  }
...
generated quantities{
  ...
  vector[nb_int] link = inv_logit(
    ...
    zgamma_pol[Y_array[, 4]] + lambda[Y_array[, 2]] .* SS
  );
  
  // Compute pointwise log-likelihood
  vector[nb_int] log_lik;
  for (i in 1:nb_int) {
    log_lik[i] = binomial_lpmf(Y_array[i, 1] | Y_array[i, 1], link[i]);
  }
}
