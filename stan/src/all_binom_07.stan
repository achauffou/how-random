  ...
  real partial_sum(
    ...
    for (i in 1:(end - start + 1)) {
      lp += bernoulli_logit_lpmf(
        ...
        zgamma[sp1_id[i]] * zgamma[sp2_id[i]] + lambda[site_id[i]] * SS[start + i - 1]
      );
    }
    return lp;
  }
...
generated quantities{
  ...
  vector[nb_int] link = inv_logit(
    alpha + beta[Y_array[, 2]] + to_vector(sigma_gamma[site_type[Y_array[, 2]]]) .* 
    zgamma[Y_array[, 3]] .* zgamma[Y_array[, 4]] + lambda[Y_array[, 2]] .* SS
  );
