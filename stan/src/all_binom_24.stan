functions{
  ...
  real partial_sum(
    ...
    vector lambda, vector SS, int[] site_type, int[] sp_group, vector mu
  ) {
    real lp = 0.0;
    ...
    int sp2_nat[end - start + 1] = y_slice[, 7];
    for (i in 1:(end - start + 1)) {
      lp += binomial_logit_lpmf(
        ...
        sp2_nat[i] * mu[sp_group[sp2_id[i]]]
      );
    }
    return lp;
  }
...
data{
  ...
  int Y_array[nb_int, 7]; // Response variable, site IDs, species IDs, replicates, origin status
...
generated quantities{
  ...
  vector[nb_int] link = inv_logit(
    ...
    gamma[Y_array[, 4]] + lambda[site_type[Y_array[, 2]]] .* SS + 
    mu[sp_group[Y_array[, 3]]] .* to_vector(Y_array[, 6]) + 
    mu[sp_group[Y_array[, 4]]] .* to_vector(Y_array[, 7])
  );
