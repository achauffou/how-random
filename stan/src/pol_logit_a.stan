functions{
  ...
  real partial_sum_lpmf(
    ...
    vector beta_pla, vector lambda, vector nu, real[] S, real[] D, 
    int[] site_id, int[] pol_id, int[] pla_id
  ) {
    real lp = 0.0;
    for (i in start:end) {
      lp += binomial_logit_lpmf(
        ...
        lambda[site_id[i]] * S[i] + nu[site_id[i]] * D[i]
      );
    }
    return lp;
  }
...
data{
  ...
  real S[nb_int]; // Suitability variable
  real D[nb_int]; // Degree variable
...
model{
  ...
  target += reduce_sum(
    partial_sum_lpmf, Y, grainsize, alpha, beta_pol, beta_pla, lambda, nu, S, D, 
    site_id, pol_id, pla_id
  );
